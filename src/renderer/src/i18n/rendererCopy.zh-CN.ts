...
export const zhCN = {
	...
	"settings.composerStatusLine": "输入框下方扩展状态行",
	"settings.composerStatusLineDesc":
		"在输入卡下方显示 pi TUI 底栏的最后一行（扩展通过 ctx.ui.setStatus 写入的配额/用量/上下文/任务进度等），按 pi 自身的排序与分隔规则拼接。「打开」不为它启动 pi 进程：只有该会话已有活进程时才会出现（纯浏览历史不起进程）；「打开并预热」会在打开会话时激活 pi 运行进程，点开就能看到，代价是为每个打开的会话起/复用进程。没有状态条目时不占高度。",
	"settings.composerStatusLineOff": "关闭",
	"settings.composerStatusLineOn": "打开",
	"settings.composerStatusLinePrewarm": "打开并预热",
