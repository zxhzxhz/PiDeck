...
export const zhCN = {
	...
	"settings.showComposerStatusLineDesc": "显示 pi TUI 底栏的最后一行：扩展通过 ctx.ui.setStatus 写入的状态（配额/用量/上下文/任务进度等），按 pi 自身的排序与分隔规则拼接。开启后打开会话会预热 pi 运行进程（状态由扩展产生，必须有活着的进程才会出现）；没有状态条目时不占高度。",
