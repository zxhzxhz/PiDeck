 ...
 *
 * 因此这里把它做成显式的成本选择：**只在用户自己打开该开关时**预热一次，
 * 每个会话每次挂载只请求一次（ref 记录，与 prewarm 同款护栏）；
 * 已有活 runtime 时不重复请求（激活本身幂等，但没必要多走一趟 IPC）。
 * 请求失败静默处理：状态行缺失可接受，不能因为一个展示项弹错误提示。
 */
export function useComposerStatusLineActivation(options: { sessionId: string; enabled: boolean; backend: AgentBackend; hasSessionRecord: boolean; runtimeLive: boolean }): void {
	const { sessionId, enabled, backend, hasSessionRecord, runtimeLive } = options;
	...
	}, [sessionId, enabled, backend, hasSessionRecord, runtimeLive]);
}
